...
interrupt(registers_t *reg)
{
  ...
  case INT_SYS_EXIT:
    ...
    while(proc != NULL)
      {
	if (proc->p_state == P_BLOCKED) // should always be true;
	  {
  ...
  case INT_SYS_WAIT: {
	...
	proc_array[p].p_state = P_EMPTY; // zombie process can be freed.
      }
    else
      {
	current->p_state = P_BLOCKED;
	process_t* proc = proc_array[p].p_waiting;
	if ( proc == NULL )
	  proc_array[p].p_waiting = current;
	else
	  {
	    while ( proc->p_waitNext != NULL )
	      {
		proc = proc->p_waitNext;
	      }
	    // at this point proc->waitNext == NULL
	    proc->p_waitNext = current;
	  }
...
do_fork(process_t *parent)
{
  ...
  // if we reach this point, we found a process! :D
  // avail now contains an empty process. 

  // now we have to copy content of stacks and registers
  ...
  avail->p_registers.reg_eax = 0; // means the child returns 0.
  return avail->p_pid;
}
