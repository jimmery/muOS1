...
struct process {
  ...
  process_t* p_waiting;         // Process wait queue. Denotes the head of
                                // a linked-list of processes in a queue
                                // waiting for this process to finish
  process_t* p_waitNext;        // denotes the next process also waiting
                                // for the same process as this process. 
                                // process is not waiting on current proc.
                                // works as a proc only waits on one proc.
};
